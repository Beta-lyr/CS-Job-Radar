...
def safe_log_crawl(session, source_id: int, status: str, fetched: int, inserted: int, skipped: int = 0, error: str = ""):
    try:
        if not session.is_active:
            session.rollback()
        log_crawl(session, source_id, status, fetched, inserted, skipped, error)
        session.commit()
    except Exception:
        traceback.print_exc()
        session.rollback()
